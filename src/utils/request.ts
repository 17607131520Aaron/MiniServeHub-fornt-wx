...
// 导出请求方法
const get = <T, R>(config: IRequestConfig<T>): Promise<R> =>
  requestMethod<T, R>("GET", config);

const post = <T, R>(config: IRequestConfig<T>): Promise<R> =>
  requestMethod<T, R>("POST", config);

const put = <T, R>(config: IRequestConfig<T>): Promise<R> =>
  requestMethod<T, R>("PUT", config);

const del = <T, R>(config: IRequestConfig<T>): Promise<R> =>
  requestMethod<T, R>("DELETE", config);

// 文件上传方法
const uploadSingleFile = async <T>(
  config: IRequestConfig<File | Blob>
): Promise<T> => {
  if (!config.data) {
...
export { del, get, post, put, uploadFile, uploadSingleFile };
